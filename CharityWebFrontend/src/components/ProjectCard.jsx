...
export default function ProjectCard({ project, onClick }) {
  ...
  return (
    <div className="project-card bg-white rounded-xl shadow-lg overflow-hidden cursor-pointer" onClick={()=>onClick(project)}>
      ...
      <div className="p-6">
        ...
        <div className="flex justify-between items-center text-sm">
          ...
          <span className="text-gray-500">{project.supporters} ủng hộ</span>
        </div>
        <div className="mt-2 text-sm">
          {project.isActive ? (<span className="text-orange-600">⏰ Còn {project.daysLeft} ngày</span>) : (<span className="text-red-600">❌ Đã kết thúc</span>)}
        </div>
      </div>
    </div>
  );
